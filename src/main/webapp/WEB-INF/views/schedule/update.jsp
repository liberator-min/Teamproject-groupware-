...
<div id="schedule_insert_box">
	<div id="schedule_insert_box_wrap">
		<div class="schedule_insert_title">제목</div>	
			<div class="schedule_insert_title_input"><input type="text" name="title" placeholder="제목을 입력해주세요" value="${schedule.title }"/></div>
		...
		<div class="schedule_insert_start_date">시작일</div>
		    <div class="schedule_insert_start_date_input"><input type="date" name="start_date" value="${schedule.start_date }"></div>
		<div class="schedule_insert_end_date">종료일</div>
		  	<div class="schedule_insert_end_date_input"><input type="date" name="end_date"value="${schedule.start_date }"></div>
		<div class="schedule_insert_content">내용</div>
			<div><textarea class="schedule_insert_content_input" rows="5" cols="5" name="content" placeholder="내용을 입력해주세요">${schedule.content }</textarea></div>
	</div>
	<div class="schedule_update_wrap">
		<button id="schedule_update" onclick="update()">수정</button>
	</div>
</div>
...
function update(){
	...
	if(title.trim() === "" || content.trim() === "" || writer.trim() === "" || type.trim() === "" || department.trim() === "" || start_date.trim() ==="" ||  end_date.trim() === ""  ){
		...
		$.ajax({
		       type: 'POST',
		       url: '${cpath}/schedule/update/${schedule.schedule_idx}',
		       data:{ title : title,      
					content : content,
					writer : writer,
