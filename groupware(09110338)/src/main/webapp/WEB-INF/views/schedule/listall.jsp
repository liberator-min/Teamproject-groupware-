...
<div class="schedule_list_wrap">
	<c:if test="${login.job_id >= 6}">
		<button class="schedule_Group_insert_btn" onclick="OpenWindow('${cpath}/schedule/insert','작성',800,800);">등록하기</button>
	 </c:if>
	 <c:if test="${login.job_id <= 5 }">
	<a href="${cpath}/schedule/list"><button class="my_group_schedule_btn">MY DEPERTMENT SCHEDULE</button></a>
	</c:if>
</div>
<div id="calendar"></div>
</div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      eventClick: function(arg) {
        ...
        var url;
        if (idx) {
        	  url = '${cpath}/schedule/selectallone/'+idx;
        	  console.log(url)
        	} 
		if(!idx){
